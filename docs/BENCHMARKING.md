...
The first harness supports Jev through the direct TypeSafe SDK, an OpenAI Responses API baseline
with strict JSON Schema output, and a fixed rule baseline. It records failures as incorrect and
includes their elapsed time in end-to-end latency. Provider order alternates by case and
repetition; execution is sequential. Retries are disabled for both network providers. Paired
confidence intervals resample cases, averaging repetitions within each case to avoid treating
repeat calls as independent examples.

The 100-case synthetic dataset is useful for smoke tests only. The realistic human-labeled and
adversarial dataset, calibration study, at least 30 timed repetitions, cost calculation from
verified prices, and immutable result publication remain future work. No result generated by the
current harness is marked eligible for a public performance claim.
